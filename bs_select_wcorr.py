...
def run(options):
    ...
    jets.Add("top_vect",    "top_index == 0 ? lead_vect : sublead_vect")
    ...
    ###############################################
    # Corrections and uncertainties (if not data) #
    ###############################################
    topSF = Correction("TopTagSF","modules/top_sf.cc",constructor=[options.year,'"medium"','"PUPPI"',"true","true"])
    a.AddCorrection(topSF)
